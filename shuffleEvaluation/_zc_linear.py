...
if __name__ == '__main__':
    ...
    for br in br_list:
        with open(r"ResultImpossibleDifferential/{}_branch_idc_filtered.pkl".format(br), 'rb') as f:
            ...
            if all(abs(v[i] - border[i]) < 3 for i in range(3)):
                for t_round in range(5, 30):
                    ...
                    if not flag:
                        result[sk] = v + [t_round]
                        break
        save_file(result, r'ResultZCLinear/{}_branch_zc_filtered'.format(br), False)
        # for s in result:
        #     print(s, ': ', result[s], ',')
